...
"""
Gather the URL fragments of duplicated official maps and create a dict structure for downstream use.
Paste the results into the OFFICIAL_MAPS_COPY constant in constants.py.
...
for xx, plan_type_urls in OFFICIAL_MAPS.items():
    print(f'    "{xx}": {{')
    for plan_type, url in plan_type_urls.items():
        url = unknown_url
        if plan_type == "congress":
            print(f'        "{plan_type}": "{unknown_url}",')
        else:
            log_path: str = (
